...
export function ClusterChecker({ children }: { children: ReactNode }) {
  ...
  if (query.isError || !query.data) {
    return (
      <div className="alert alert-warning flex justify-center rounded-none text-warning-content/80">
        <span>
          Error connecting to cluster <strong>{cluster.name}</strong>
        </span>
        <button className="btn btn-neutral btn-xs" onClick={() => query.refetch()}>
          Refresh
        </button>
      </div>
    );
...
export function ClusterUiTable() {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="table border-separate border-4 border-base-300">
        <thead>
          <tr>
          ...
          {clusters.map((item) => (
            <tr key={item.name} className={item?.active ? 'bg-base-200' : ''}>
              <td className="space-y-2">
                ...
                <div className="whitespace-nowrap text-xs text-gray-500">{item.endpoint}</div>
              </td>
              <td className="space-x-2 whitespace-nowrap text-center">
                <button
                  disabled={item?.active}
                  className="btn-default btn btn-outline btn-xs"
                  onClick={() => {
